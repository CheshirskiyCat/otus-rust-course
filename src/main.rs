...
const WRONG_COORDINATE: &'static str = "Wrong data format: missing point coordinate";
const WRONG_COLOR: &'static str = "Wrong data format: missing point color";
const WRONG_PARAMETER: &'static str = "Wrong data format: undefined parameter";
const POINT_NOT_VALID: &'static str = "Point is not valid";

struct Point{
    coords: Option<(u64, u64)>,
    color: Option<u64>,
}

impl Point {

    fn check(&self, display: &Display){
        if self.coords == None || self.color == None || Point::is_out_side_point(self, display) {
            panic!("{}", POINT_NOT_VALID);
        }
    }
    fn is_out_side_point(&self, display: &Display) -> bool {
        let coords = self.coords.unwrap();
        display.height as u64 <= coords.1 || display.width as u64 <= coords.0
    }
}
...
fn process_commands(display: &mut Display, input: Vec<u64>) {
    // ваш код сюда
    let mut i = 0;
    let mut point = Point{coords: None, color: None};
    loop {
        if i >= input.len() { break }
        match input.get(i).unwrap() {
            1 => {
                point.coords = Some((
                    *input.get(i+2).expect(WRONG_COORDINATE),
                    *input.get(i+1).expect(WRONG_COORDINATE),
                ));
                i += 3;
            }
            2 => {
                point.color = Some(*input.get(i+1).expect(WRONG_COLOR));
                i += 2;
            }
    ...
    println!("finish");
    point.check(display);
    let coords = point.coords.expect(POINT_NOT_VALID);
    let color = point.color.expect(POINT_NOT_VALID) as u8;
    display.matrix.set_colour(coords.1, coords.0, color);
}
